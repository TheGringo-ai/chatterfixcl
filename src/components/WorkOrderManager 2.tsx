...
import { 
  ...
  CheckCircle, XCircle, Edit, Save, Cancel, Plus,
...
interface WorkOrderEnhanced {
  ...
  notes: {
    ...
    author: string;
    timestamp: string;
    type: 'note' | 'status_change' | 'part_checkout' | 'checkin_checkout';
  }[];
  procedures: string[];
  safetyNotes: string[];
}
...
interface WorkOrderManagerProps {
  workOrder?: WorkOrderEnhanced;
  onSave: (workOrder: WorkOrderEnhanced) => void;
  onCancel: () => void;
...
const WorkOrderManager: React.FC<WorkOrderManagerProps> = ({ 
  ...
  const [formData, setFormData] = useState<WorkOrderEnhanced>(
    workOrder || {
      id: `WO-${Date.now()}`,
      title: '',
      description: '',
  ...
  const handleTechnicianCheckIn = () => {
    ...
    if (formData.checkedInTechnician) {
      ...
      setFormData(prev => ({
        ...prev,
        checkedInTechnician: null,
        actualHours: prev.actualHours + hoursWorked
      }));
  ...
  const handleDowntimeToggle = () => {
    ...
    if (downtimeActive) {
      ...
      setFormData(prev => ({
        ...prev,
        downtime: {
          started: prev.downtime.started,
          ended: now,
          totalMinutes: prev.downtime.totalMinutes + minutes
        }
      }));
      ...
      <div className="bg-gradient-to-r from-blue-600 to-indigo-600 text-white p-6">
        ...
        <div className="grid grid-cols-2 md:grid-cols-4 gap-4 mt-4 pt-4 border-t border-blue-500">
          ...
          <div>
            ...
            <p className="font-medium">{formData.actualHours.toFixed(2)}h</p>
          </div>
        </div>
            ...
            <div className="bg-orange-50 border border-orange-200 rounded-lg p-4">
              ...
              <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
                <div>
                  ...
                  <p className="font-semibold text-orange-900">{formData.downtime.totalMinutes.toFixed(1)} minutes</p>
                </div>
                <div>
